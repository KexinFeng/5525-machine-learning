...
if ~allclose(sum(gamma_init, 2), ones(2400, 1), 1e-10)
...
while 1
    ...
    if ~allclose(sum(gamma_new, 2), ones(2400, 1), 1e-10)
        error('gamma not normalized');
    end
...
if ~allclose(sum(sum(Posteriors, 1)),N)
    error('Posteriors  not equal to N')
end
if ~allclose(sum(Posteriors, 2), ones(2400, 1), 1e-10)
    error('Posteriors not normalized');
end
