...
#     'out' : weight_variable([2*n_hidden, n_classes])
# }
# biases = {
...
weights = {
    ...
    'sum' : tf.concat([tf.eye(n_hidden), tf.eye(n_hidden)], axis=0)
}
biases = {
...
def lstm_layer(X, init_state, weights, biases ):
    ...
    last_output = tf.nn.xw_plus_b(tf.matmul(outputs[-1], weights['sum']), weights['out'], biases['out'])
    return last_output
...
output_file_name = './files_to_plot/bidi.csv'
...
plt.figure()
plt.plot(loss_t)
plt.title('loss_t_bidirection')

plt.figure()
plt.plot(acc_t)
plt.title('training_accuracy_bidirection')

plt.figure()
plt.plot(time_t)
plt.title('time_bidirection/display_step')
