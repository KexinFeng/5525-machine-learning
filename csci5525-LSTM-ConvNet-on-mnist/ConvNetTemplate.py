...
def main(argv=sys.argv):
    ...
    try:
        option = argv[1]
        if option == 'Adam':
            # remember to remove checkpoint file!
            default_opt = False
    except:
        pass
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        # kernel = tf.Variable(tf.random_normal([5, 5, 1, 32]), name='kernel')
        ...
        # biases = tf.Variale(tf.zeros([32], dtype=tf.float32), name='biases')
        # TO DO

        # apply tf.nn.conv2d. strides [1, 1, 1, 1], padding is 'SAME'
        X_times_kernal = tf.nn.conv2d(images, kernel, strides=[1, 1, 1, 1], padding='SAME')

        # TO DO

        # apply relu on the sum of convolution output and biases
        conv1 = tf.nn.relu(X_times_kernal + biases, name=scope.name)
        # TO DO
    ...
    #     pool_out = tf.nn.pool([1, 2, 2, 1], poolingtype='max', padding='SAME', strides=[1, 2, 2, 1])
    ...
    with tf.variable_scope('fc') as scope:
        ...
        w = tf.Variable(tf.random_normal([input_features, 1024]))
    ...
    # you need to create weights and biases
        weight = tf.Variable(tf.random_normal([1024, 10]), name='w_out')
        bias = tf.Variable(tf.random_normal([10]), name='b_out')
        logits = tf.nn.xw_plus_b(fc, weight, bias, name='logits')
    # TO DO
    ...
    # Attension: Although we are asked to use gradient descent here, the accuracy is as small as 0.01.
    # So I tried an alternative optimizer here, AdamOptimizer, and the accuracy is up to 0.9.
    # But the code can run with GradientDescentOptimizer.
    #################################################
    ...
    with tf.Session() as sess:
        ...
        for index in range(initial_step, n_batches * N_EPOCHS):  # train the model n_epochs times
            ...
            if (index + 1) % SKIP_STEP == 0:
                ...
                # print('epoch= ', index//(n_batches*N_EPOCHS))
                total_loss = 0.0
